...
func (s *instrumentedStore) Open(ctx context.Context, name string) (BinaryKeyspace, error) {
	...
	ks := &instrumentedKeyspace{
		Telemetry:     telem,
		OpenKeyspaces: telem.UpDownCounter("kv.open", "{keyspace}", "The number of keyspaces that are currently open."),
		Conflicts:     telem.Counter("kv.conflicts", "{error}", "The number of times setting a value has failed due to an optimistic-concurrency conflict."),
		Misses:        telem.Counter("kv.misses", "{operation}", "The number of times the value associated with a specific key was requested but not present in the keyspace."),
		KeyIO:         telem.Counter("kv.key.io", "By", "The cumulative size of the keys that have been operated upon."),
		ValueIO:       telem.Counter("kv.value.io", "By", "The cumulative size of the values that have been operated upon."),
		KeySize:       telem.Histogram("kv.key.size", "By", "The sizes of the keys that have been operated upon."),
		ValueSize:     telem.Histogram("kv.value.size", "By", "The sizes of the values that have been operated upon."),
	}
