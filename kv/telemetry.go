...
func (ks *instrumentedKeyspace) Get(ctx context.Context, k []byte) ([]byte, []byte, error) {
	keySize := int64(len(k))
	...
	if valueSize != 0 {
		...
		span.SetAttributes(
			...
			telemetry.Binary("concurrency_token", t),
		)
...
func (ks *instrumentedKeyspace) Set(ctx context.Context, k, v, t []byte) ([]byte, error) {
	keySize := int64(len(k))
	valueSize := int64(len(v))
	...
	ctx, span := ks.Telemetry.StartSpan(
		...
		telemetry.Binary("key", k),
		telemetry.Int("key_size", keySize),
		telemetry.Binary("concurrency_token.before", t),
	)
	defer span.End()
	...
	if valueSize == 0 {
		...
		span.SetAttributes(
			telemetry.Binary("concurrency_token.after", t),
		)
...
func (ks *instrumentedKeyspace) Range(ctx context.Context, fn BinaryRangeFunc) error {
	...
	err := ks.Next.Range(
		ctx,
		func(ctx context.Context, k, v, t []byte) (bool, error) {
			count++
